...
namespace HM_Cdss.HMUtil
{
    ...
    public class TCPServer
    {
        ...
        /// 默认的服务器最大连接客户端端数据
        /// </summary>
        public const int UnicodeMaxClient = 100;
        /// <summary>
        /// 接收数据缓冲区大小64K
        /// </summary>
        ...
        public TCPServer(ushort port, ushort maxClient, Coder coder)
        {
            _port = port;
        ...
        // <summary>
        /// 构造函数(默认使用UTF8编码方式和UTF8MaxClient(100)个客户端的容量)
        /// </summary>
        /// <param name="port">服务器端监听的端口号</param>
        public TCPServer(ushort port)
            : this(port, UnicodeMaxClient)
        {
        }
        // <summary>
        /// 构造函数(默认使用UTF8编码方式和UTF8MaxClient(100)个客户端的容量)
        /// </summary>
        /// <param name="port">服务器端监听的端口号</param>
        public TCPServer(ushort port, Coder coder)
        {
            _port = port;
            _maxClient = UnicodeMaxClient;
            _coder = coder;
        }
        #endregion
        ...
        protected virtual void ReceiveData(IAsyncResult iar)
        {
            ...
            try
            {
                ...
                byte[] receiveBytes = new byte[recv - 1];
                Array.Copy(_recvDataBuffer, 1, receiveBytes, 0, recv - 1);
                ...
                if (RecvData != null)
                {
                    ...
                    Trace.Assert(sendDataSession != null);
    ...
    public class TCPClient
    {
        ...
        /// 接收数据缓冲区
        /// </summary>
        public byte[] _recvDataBuffer = new byte[DefaultBufferSize];
        public int _recvDataLength = 0;

        //private NetworkStream FobjStream = null;
        ...
        public virtual void Send(byte[] ASendBytes, int AlOffset, int AlDataLen)
        {
            if (!_isConnected)
            {
                //throw (new ApplicationException("没有连接服务器，不能发送数据"));
            ...
            try
            {
                /*NetworkStream FobjStream = new NetworkStream(_session.ClientSocket);
                FobjStream.Write(ASendBytes, AlOffset, AlDataLen);
                FobjStream.Close();
                FobjStream.Dispose();*/
                _session.ClientSocket.BeginSend(ASendBytes, AlOffset, AlDataLen, SocketFlags.None, new AsyncCallback(SendDataEnd), _session.ClientSocket);
            }
            catch (Exception e)
            {
                //throw e;
        ...
        protected virtual void RecvData(IAsyncResult iar)
        {
            ...
            try
            {
                ...
                _recvDataLength = recv;
                string receivedData = _coder.GetEncodingString(_recvDataBuffer, recv);
                //receivedData = "";
                //for (int i = 0; i < recv; i++)
                //{
                //    receivedData += _recvDataBuffer[i] + ";";
                //}
    ...
    /// 本类的实现的是标记符的方法,你可以在继承类中实现其他的方法
    /// </summary>
    public class DatagramResolver
    {
        public enum METHOD    //解析方法
        {
            START_TAG,             //起始符
            END_TAG,               //结束符
            VAR_LENGTH,            //可变长度
            FIXED_LENGTH,          //固定长度
            START_AND_LENGTH,      //起始符和可变长度
        }
        ...
        /// 报文长度信息在报文中的位置
        /// </summary>
        private int indexOfLength = 0;

        private int lPaketLength = 8;

        private METHOD _ResolveMethod = METHOD.END_TAG;
        ...
        public int DatagramIndexLength
        {
            get { return indexOfLength; }
            set { indexOfLength = value; }
        }


        public int DatagramLength
        {
            get { return lPaketLength; }
            set { lPaketLength = value; }
        }

        public METHOD DatagramMethod
        {
        ...
        /// 构造函数
        /// </summary>
        /// <param name="indexLength">长度信息在报文中的位置</param>
        public DatagramResolver(int indexLength)
        {
            if (indexOfLength < 0)
            {
                //throw (new ArgumentException("报文长度索引不能<0"));
                return;

            }
            this.indexOfLength = indexLength;
        }
        ...
        /// 解析报文
        /// </summary>
        /// <param name="rawDataBytes">原始报文字节数组</param>
        /// <returns></returns>
        public virtual byte[][] Resolve(ref byte[] rawDataBytes)
        {
            ArrayList datagrams = new ArrayList();
            int lPackageCount = 0;  //数据帧的有效数量
            //int lIndex = 0;         //当前查询索引
            switch (DatagramMethod)
            {
                case METHOD.VAR_LENGTH:
                    //while (true)
                    //{
                    //    tagIndex = rawDatagram.IndexOf(endTag, tagIndex + 1);
                    //    if (tagIndex == -1)
                    ...
                    while (true)
                    {
                        if (rawDataBytes.Length < indexOfLength + 1)
                        {
                            break;  //未满一个完整帧
                        }
                        else
                        {
                            if (rawDataBytes.Length < indexOfLength + 1 + rawDataBytes[indexOfLength])
                                break; //未满一个完整帧
                            else
                            {
                                byte[] bOnePackage = new byte[indexOfLength + 1 + rawDataBytes[indexOfLength]];
                                Array.Copy(rawDataBytes, 0, bOnePackage, 0, bOnePackage.Length);
                                byte[] bReservedData = new byte[rawDataBytes.Length - bOnePackage.Length];
                                Array.Copy(rawDataBytes, bOnePackage.Length, bReservedData, 0, bReservedData.Length);
                                Array.Resize(ref rawDataBytes, rawDataBytes.Length - bOnePackage.Length);
                                bReservedData.CopyTo(rawDataBytes, 0);
                                //lIndex+=bOnePackage.Length;
                                lPackageCount++;
                                datagrams.Add(bOnePackage);
                            }
                        }
                    }
                    break;

                case METHOD.FIXED_LENGTH:  //固定长度
                    while (true)
                    {
                        if (rawDataBytes.Length < DatagramLength)
                        {
                            break;  //未满一个完整帧
                        }
                        else
                        {
                            byte[] bOnePackage = new byte[DatagramLength];
                            Array.Copy(rawDataBytes, 0, bOnePackage, 0, bOnePackage.Length);
                            byte[] bReservedData = new byte[rawDataBytes.Length - bOnePackage.Length];
                            Array.Copy(rawDataBytes, bOnePackage.Length, bReservedData, 0, bReservedData.Length);
                            Array.Resize(ref rawDataBytes, rawDataBytes.Length - bOnePackage.Length);
            ...
            byte[][] results = new byte[lPackageCount][];
            datagrams.CopyTo(results);
            return results;
    ...
    public class Session : ICloneable
    {
        ...
        object System.ICloneable.Clone()
        {
            ...
            newSession.DataBytes = _dataBytes;
